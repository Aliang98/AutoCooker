...
if localization_TC.config.other.allow_cooker then
	if global_toggle_meth then
		...
		--managers.mission._fading_debug_output:script().log(string.format("%s %s", bag_amount, lang.menu_button_morem_enable),  Color.gold)
		managers.chat:_receive_message(1, lang.menu_button_spawn_cooker, string.format("%s %s", bag_amount, lang.menu_button_morem_enable), Color.gold)
	else
